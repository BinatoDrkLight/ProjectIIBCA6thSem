...
const glickoTwo = async ({ winnerId, loserIds, weightForWin, weightForLoss, userId, from }) => {
  try {
    // Fetch winner & losers
    ...
    losers.forEach((loser) => {
      ...
      const adjustedScore = expected + weightForWin * (1 - expected); // winner = 1
    ...
    const newMuWinner = muWinner + newPhiWinner * newPhiWinner * deltaWinner;
  ...
  await Product.findByIdAndUpdate(
    ...
    for (const loser of losers) {
      ...
      const adjustedScore = expected + weightForLoss * (0 - expected); // loser score = 0
